...
from csflix.domain.model import User, Article, Tag, Comment, make_comment
...
def test_repository_can_get_articles_by_ids(in_memory_repo):
    ...
    assert len(articles) == 3
    assert articles[
               0].title == 'csflix 19 coronavirus: US deaths double in two days, Trump says quarantine not necessary'
